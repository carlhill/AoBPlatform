...
- **Choose tech for iteration speed (Carl, 3 Sep 2026).** Before adopting a build path, measure the edit-to-verify loop. A stack where every look costs a minute-plus build (the Expo kiosk: `expo export` per view, Jest with React Native mocks) wastes wall clock and tokens on every agent turn; a stack with hot reload and the test runner the rest of the repo already uses does not. Prefer what the monorepo already runs (Next.js, Vitest, Playwright) over a second toolchain, and treat "a look costs a build" as a defect in the choice, not a cost of doing business. Recorded decisions still apply (aob-tech-stack.md) -- change them deliberately, as the kiosk was.
- **Option lists are content, not code (Carl, 3 Sep 2026).** Any list a user picks from -- relationships, reasons, categories, states -- lives in a versioned content file under `packages/domain/content/`, schema-validated at load, with display labels in the string table keyed by option key. A screen renders the file in file order and never carries the list itself, so the list changes without a code change and the version travels with the record it produced. First instance: `assignor-relationships.json`.
- **Shortcuts to the answer, not directions to a screen (Carl, 4 Sep 2026).** When the platform tells a user something is blocked, wrong or waiting, the message carries the specific reason and a link that lands on the exact item where it is fixed -- never "see the practice queue" in prose. A generic fallback message is a defect: every server reason code maps to copy plus a destination, and an unmapped code shows its code so it can be diagnosed. First instance: "Cannot be sent yet" on /practice/tablet.
