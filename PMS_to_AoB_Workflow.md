...
|---|---|---|---|---|
| W1 | **Renderer carries the full s 65C data set + practice letterhead** | 1, 3, 4 | **BUILT** 5 Sep 2026 | `pdf-2` renders the whole document -- letterhead (legal and trading name, address, phone, email, ABN, optional logo), D1-D7 or reg 65CB's set, and the statements the assignor ticks -- from `packages/domain/content/agreement-templates.json`; the document is stored and hashed, so correcting a rendered element now moves the bytes. Per-practice wording is proposed at `/practice/templates` and activated only at `/platform/templates`. **7 Sep 2026:** that proposal is now a FORM, not a JSON textarea (Carl: "make this custom text a form and then you put it together in the right JSON format") -- the generic's shape as fields, placeholders inserted from a picker built off the content file's declared list, "only when" wrappers around the selection, live checks from the same loader the server runs (Save and Submit dead while any fails, every reason shown), and a sample-data preview marked "Preview -- not the signed document". **The generic wording is a DRAFT pending Carl and counsel** (§5 Q3) |
| W2 | **Case 4 — "New agreement" form on the work page / queue**: reception types the elements (patient found-or-created by record number; provider; visit) → the arrival pipeline. Reuses the arrival service with `source: 'reception'` | 4 | **BUILT** 7 Sep 2026 | A "New agreement" panel on `/practice/patients` and on a patient's work page posts the SAME `POST /arrivals` with `source: 'reception'` — one pipeline, one visit policy, one lock, one queue row — with `POST /arrivals/preview` showing what the visit needs before Submit (reception never chooses the type, hard rules 6 and 14) and `GET /patients/search` finding somebody the practice already holds without becoming a directory. Who signs is set between the capture request and the lock, through the existing assignor service, so hard rule 10 is enforced once. The row records the staff principal. **Open for Carl:** the 18+ answer is a declaration, not a date of birth — see TODO.md §2a |
...
| W6 | **"Send me a copy" for the patient**: after signing, offer email/SMS with a link to the signed PDF (the portal already serves it); a printed copy at the desk | 1 | **SEND HALF BUILT** 11 Sep 2026 | K-6 offers the copy to the contact ALREADY ON THE RECORD, shown masked (`j•••@e•••.com`), and never accepts a typed one — D-2026-09-11-03, and there is no input on the screen and no field on the API that one could arrive in. Whose contact is decided by `assignorIsPatient`: the patient record when the patient signed, that assignor's own when somebody else did (ASSIGNOR-RULES rules 5-6). The channel list is versioned content (`packages/domain/content/copy-delivery-channels.json`) and the words are the string table, keyed by option key. The message carries a LINK, never an attachment: `GET /agreement-copy/:token` re-renders under the renderer version on the agreement and compares hashes before a byte leaves (hard rule 13, the same `verifiedArtefactOf` the portal runs). The copy link, the queued message, its correspondence twin and `agreement.copy_requested` commit in one transaction (hard rule 11); the event carries ids, channel TYPES and versions and never an address (hard rule 9). A decline, a failed send and a missing contact all leave the ceremony complete (hard rule 8) and surface at the desk in `/practice/correspondence`, never on the patient's screen. **Open for Carl:** the link opens with NO identity check and lasts 7 days, opening as many times as the holder likes — it was sent to the address the record already held, opens one already-signed document and grants nothing else, but it is a deliberate departure from the portal's three-identifier check and is worth confirming. **NOT BUILT: the printed copy at the desk.** It needs a staff artefact route and a console surface listing SIGNED agreements, which does not exist yet (the work page lists pushable rows). Half a day on its own |
...
| Q9 | When somebody other than the patient signs, do we capture the assignor's **date of birth**, or record their **declaration** that they are 18 or over? | The declaration, as REQ-AGE-01/REQ-VUL-02 already require and as the kiosk, the tablet desk and `buildAssignorForAnother` already do. W2's brief asked for a DOB; that would open a new store of a birth date for a non-patient and contradict three existing named tests, so it was not built. Reopen with a recorded decision if the regulator's reading has changed |
...
| 0.4 | 7 Sep 2026 | W2 BUILT: reception types an arrival by hand (`source: 'reception'`), with `POST /arrivals/preview` and `GET /patients/search` behind it. `typed_and_arrived_agreements_share_one_pipeline` is answered by `reception_arrival_runs_the_same_pipeline_as_a_pms_arrival`. Q9 raised: the assignor's 18+ answer is a declaration, not a date of birth. |
| 0.3 | 7 Sep 2026 | W1 wording editor is a structured form with live s 65C checks and a sample-data preview; the domain loader gained `agreementTemplateProblems` so the console and the server run one set of rules. |
